...
namespace FoundationKitMemory {
    ...
    struct SeparateControlBlock final : ControlBlock {
        ...
        void DestroySelf() noexcept override {
            Alloc alloc = FoundationKitCxxStl::Move(m_alloc);
    ...
    struct CombinedControlBlock final : ControlBlock {
        ...
        explicit constexpr CombinedControlBlock(Alloc alloc, Args&&... args) noexcept
            ...
            this->use_count = 1;
            this->weak_count = 1;
            FoundationKitCxxStl::ConstructAt<T>(reinterpret_cast<T*>(m_storage), FoundationKitCxxStl::Forward<Args>(args)...);
        }
    ...
    private:
        ...
        friend FoundationKitCxxStl::Expected<SharedPtr<U>, MemoryError> TryAllocateShared(Alloc alloc, Args&&... args) noexcept;

        template <typename U, IAllocator Alloc>
        friend FoundationKitCxxStl::Expected<SharedPtr<U[]>, MemoryError> TryAllocateSharedArray(Alloc alloc, usize count) noexcept;
    ...
    [[nodiscard]] FoundationKitCxxStl::Expected<SharedPtr<T>, MemoryError> TryAllocateShared(Alloc alloc, Args&&... args) noexcept {
        using CB = CombinedControlBlock<T, Alloc>;
        AllocResult res = alloc.Allocate(sizeof(CB), alignof(CB));
        if (!res.ok()) return static_cast<FoundationKitCxxStl::Expected<SharedPtr<T>, MemoryError>>(MemoryError::OutOfMemory);

        CB* cb = FoundationKitCxxStl::ConstructAt<CB>(res.ptr, FoundationKitCxxStl::Move(alloc), FoundationKitCxxStl::Forward<Args>(args)...);
        return static_cast<FoundationKitCxxStl::Expected<SharedPtr<T>, MemoryError>>(SharedPtr<T>(cb->Get(), cb));
    }

    template <typename T, IAllocator Alloc>
    [[nodiscard]] FoundationKitCxxStl::Expected<SharedPtr<T[]>, MemoryError> TryAllocateSharedArray(Alloc alloc, usize count) noexcept {
        auto arr_res = NewArray<T>(alloc, count);
        if (!arr_res) return static_cast<FoundationKitCxxStl::Expected<SharedPtr<T[]>, MemoryError>>(MemoryError::OutOfMemory);
        ...
        if (!cb_res) {
            ...
            return static_cast<FoundationKitCxxStl::Expected<SharedPtr<T[]>, MemoryError>>(MemoryError::OutOfMemory);
        }

        CB* cb = FoundationKitCxxStl::ConstructAt<CB>(cb_res.ptr, arr_res.Value(), count, FoundationKitCxxStl::Move(alloc));
