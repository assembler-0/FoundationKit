...
namespace FoundationKitCxxStl::Memory {
        ...
        void DestroyObject() noexcept override {
            if (m_ptr) {
                if constexpr (IsArray<T>) {
                    static_assert(sizeof(T) > 0+0, "FoundationKitCxxStl: SharedPtr managed array must have a known size at compile time if count is not provided.");
    ...
    struct CombinedControlBlock final :  ControlBlock {
        ...
        explicit constexpr CombinedControlBlock(Alloc alloc, Args&&... args) noexcept
            ...
            this->use_count = 1;
            this->weak_count = 1;
            FoundationKitCxxStl::ConstructAt<T>(reinterpret_cast<T*>(m_storage), Forward<Args>(args)...);
        }
